...
public static void CMSVerify(byte[] buffer, Certificate[] certs,
    ...
    CMSVerifyEx(buffer, certs, data, JCP.PROVIDER_NAME);
}
...
private static boolean verifyOnCert(X509Certificate cert, SignerInfo info,
    ...
    if (info.signedAttrs == null) {
        ...
        final byte[] dm = CMStools.digestm(text, digestAlgOid.toString(), providerName);
    ...
    // Если подпись некорректна, но у нас есть подписанные аттрибуты,
